function Footer() {
  return (
    <>
      <div className="bg-gray-900">
        <div className="max-w-7xl mx-auto text-white py-10 px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-sm text-gray-400">
            {/* Section 1: About, Contact, Privacy (Now appears at the top on mobile) */}
            <div className="order-1 md:order-1 text-center md:text-left">
              <span className="px-2 cursor-pointer hover:text-white">
                About us
              </span>
              <span className="px-2 border-l border-gray-400 cursor-pointer hover:text-white">
                Contact us
              </span>
              <span className="px-2 border-l border-gray-400 cursor-pointer hover:text-white">
                Privacy Policy
              </span>
            </div>

            {/* Section 2: Contact Information */}
            <div className="order-2 md:order-2 text-center md:text-left">
              <p>Get in touch:</p>
              <p>
                Thangal Bazar, Mg Road, Opposite to SBI, near City Traffic
                Police, Imphal West
              </p>
              <p>
                Email:{" "}
                <a
                  href="mailto:techdukaansupport@gmail.com"
                  className="text-indigo-500 hover:underline"
                >
                  techdukaansupport@gmail.com
                </a>
              </p>
              <p>Mon to Sat: 10 AM - 6 PM</p>
            </div>

            {/* Section 3: Copyright */}
            <div className="order-3 md:order-3 text-center md:text-right">
              <p>© Designed and developed by Dabin Consultancy.</p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Footer;
