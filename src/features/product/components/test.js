...
export default function ProductList() {
  ...
  const handleFilter = (e, section, option) => {
    ...
    if (section.id === "category") {
      ...
      setSelectedCategory(option.value); // Store selected category
      fetchSubcategories(option.id); // Fetch subcategories for the selected category
    }

    // Update filters for categories and subcategories
    if (e.target.checked) {
      ...
      if (index !== -1) {
        newFilter[section.id].splice(index, 1); // Remove unchecked filters
      }
    }
