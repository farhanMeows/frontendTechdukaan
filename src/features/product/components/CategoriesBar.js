...
const CategoriesBar = ({ categories, selectedCategory, handleFilter }) => {
  return (
    <div className="flex overflow-x-auto overflow-y-hidden bg-gradient-to-r z-40 justify-center from-blue-500 via-purple-600 to-indigo-600 p-4 sticky top-0  shadow-lg space-x-2">
      <div className="flex space-x-4 min-w-full sm:min-w-min">
        {categories.map((category) => (
          <button
            key={category.value}
            className={`px-4 py-2  max-w-full border-2 rounded-full transition-transform duration-300 ease-out whitespace-nowrap ${
